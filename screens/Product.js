...
class Product extends Component {
  ...
  async handlePress(id, name) {
    const stringId = JSON.stringify(id);
    try {
      await AsyncStorage.setItem(stringId, name);
    } catch (error) {
      console.log(error);
    }
    return Alert.alert("Added to cart!", "Shop for more!", [
      { text: "OK", onPress: () => console.log(name) },
    ]);
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Block style={styles.product}>
          ...
          <Block flex={false} row margin={[theme.sizes.base, 0]}>
            ...
            <TouchableOpacity
              onPress={() => this.handlePress(product.id, product.name)}
            >
              <Text
                style={[
                  styles.tag,
